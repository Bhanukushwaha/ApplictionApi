class FollowsController < ApplicationController
  ...
  def following
    ids = Follow.where(current_user_id:current_user.id)
    following = User.where(id:ids)
    render json: following, each_serializer: FollowSerializer
  end

  def followers
    ids = Follow.where(user_id:current_user.id)
    followers = User.where(id:ids)
    render json: followers, each_serializer: FollowSerializer
  end
  def unfollow
